...
export default function EmployeeTeamStatusPage() {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403) {
          ...
          if (data.error === "این قابلیت غیرفعال است") {
            setNoAccess(true);
            setError("این قابلیت توسط ادمین غیرفعال شده است");
          } else {
      ...
      // اگر کاربری نیست، یعنی دسترسی ندارد
      if (!data.users || data.users.length === 0) {
        if (!data.departments || data.departments.length === 0) {
          setNoAccess(true);
          setError("شما دسترسی به مشاهده وضعیت تیم را ندارید");
          return;
        }
      }
  ...
  if (status === "loading" || loading) {
    return (
      <MobileLayout role="EMPLOYEE" title="وضعیت تیم">
    ...
    return (
      <MobileLayout role="EMPLOYEE" title="وضعیت تیم">
        <div className="flex flex-col items-center justify-center h-64 text-center px-4">
          ...
          <p className="text-gray-500">{error || "دسترسی غیرمجاز"}</p>
          <button
            onClick={() => router.push("/mobile/employee")}
