  ...
  
  {
    ...
    if (createdBy && department) {
      const poll = await prisma.polls.create({
        data: {
          id: randomUUID(),
          title: "نظرسنجی رضایت از سیستم فیدبک",
          description: "لطفاً نظر خود را درباره سیستم فیدبک اعلام کنید",
          type: "SINGLE_CHOICE",
      ...
      console.log(`✅ نظرسنجی "${poll.title}" ایجاد شد`);
    }
  }

  {
    const createdBy = createdUsers.find((u) => u.mobile === "09123322111");
    const department = createdDepartments.find((d) => d.name === "IT");
    if (createdBy && department) {
      const poll = await prisma.polls.create({
        data: {
          id: randomUUID(),
          title: "نظرسنجی درباره کیفیت غذا",
          description: "نظر خود را درباره کیفیت غذا در آشپزخانه اعلام کنید",
          type: "RATING_SCALE",
          visibilityMode: "PUBLIC",
          isActive: true,
          allowMultipleVotes: false,
...

  {
    ...
    if (createdBy) {
      const poll = await prisma.polls.create({
        data: {
          id: randomUUID(),
          title: "نظرسنجی عمومی درباره محیط کار",
          description: "نظر خود را درباره محیط کار و فضای اداری اعلام کنید",
          ...
          poll_options: {
            create: [
              {
                id: randomUUID(),
                text: "محیط کار مناسب است",
                order: 1,
              },
              {
                id: randomUUID(),
                text: "فضای اداری نیاز به بهبود دارد",
                order: 2,
              },
              {
                id: randomUUID(),
                text: "تجهیزات کافی نیست",
                order: 3,
              },
              {
                id: randomUUID(),
                text: "نور و تهویه مناسب است",
                order: 4,
              },
            ],
          },
